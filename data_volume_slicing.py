def data_volume_slicing(path):
    ...
    #calculate origin
    origin_up = np.array([32,65,120])  #taken where intraop 0, 2, 4 exists i.e. first slice
    origin_down = np.array([85,65,120])  #taken where intraop 5,6,8,9 exists i.e. last slice
    ...
    table.SetRange(0, 905) # image intensity range
    ...
    im_actor.GetMapper().SetInputConnection(color.GetOutputPort())
    ...
    for i in range(slices):
        ...
        arr_ = vtk_to_numpy(im_vtk_array).reshape(height, width, components)
        vol_[i,:,:]  = arr_[:,:,0]
